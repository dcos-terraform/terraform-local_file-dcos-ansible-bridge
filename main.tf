...
data "template_file" "dcos_configs" {
  template = "${file("${path.module}/dcos_configs.tpl")}"

  vars {
    dcos_variant                                 = "${var.dcos_variant}"
    bootstrap_private_ip                         = "${var.bootstrap_private_ip}"
    dcos_bootstrap_port                          = "${var.dcos_bootstrap_port}"
    dcos_download_path                           = "${var.custom_dcos_download_path}"
    ...
    master_private_ips                           = "${join("\",\"", var.master_private_ips)}" 
  }
}

locals {
  dcos-config-file = "${data.template_file.dcos_configs.rendered}"
}
